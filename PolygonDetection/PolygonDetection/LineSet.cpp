...
#define ADD_NEW_SEGMENT(lines_list, p1, p2) { \
	if (!((*p1)==(*p2))) lines_list.Add(new Line(p1, p2)); \
}
...
bool PolygonDetection::LineSet::RemoveIntersections(void)
{
	...
	if (!PolygonDetection::PolygonDetector::Silent())
        printf("Detected %d intersections", intersection_count);

	size_t i, j, count = _lines_array.GetCount();
	Line *line;
	...
	for (i=0; i<count; i++) {
		...
		if (line->HaveIntersections()) {
			...
			for (j=0; j<intersections_list->GetCount(); j++){
				intersection = intersections_list->Item(j);
	...
	for (i=_lines_array.GetCount(); i>0; ) {
		line = _lines_array[--i];
...
{
	// sweep all lines
	for (size_t i=_lines_array.GetCount(); i>0; i--) {
...
unsigned int PolygonDetection::LineSet::DetectIntersections(void)
{
	...
	size_t j, counter = _lines_array.GetCount();
	...
	for(size_t n=0; n<counter;n++) {
		...
		for (j=active_line_segments_array->GetCount(); j>0;) {
			other_line = active_line_segments_array->Item(--j);
...
{
	for (size_t i=0; i< _lines_array.GetCount(); i++) 
...
{
	for (size_t i=0; i<_lines_array.Count();i++)
