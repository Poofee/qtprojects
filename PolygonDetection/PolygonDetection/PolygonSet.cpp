...
void PolygonSet::Simplify(double smaller_polygon_length)
{
	...
	j=0;
	for (i=_polygons_array.GetCount(); i>0;) {
...
//			wxDELETE(p1);
	...
	if(!PolygonDetector::Silent())
        printf("Removed %d small polygons", j);

    // check adjacencies
	for (i=0; i<_polygons_array.GetCount();i++) {
		p1 = _polygons_array[i];	
		for (j=i+1; j<_polygons_array.GetCount();j++) {			
...
bool PolygonDetection::PolygonSet::Construct(LineSet * line_set)
{
	...
	if (!PolygonDetection::PolygonDetector::Silent())
        printf("Created graph with %lld vertices and %lld edges.",
			G->GetVertexCount(),
			G->GetEdgeCount());
...
#endif 	
				if (!PolygonDetector::Silent())
                    printf("Detected %lld cycles.", cycle_set->GetCount());
...
void PolygonDetection::PolygonSet::CreatePointsArray(LineSet * line_set)
{
	...
	if (line_set) {
		...
		for (size_t i=0; i< line_set->GetCount(); i++) {
		...
		for(i=0; i<_all_points_array.GetCount();i++)
...
*/
size_t PolygonSet::GetPointCount()
{
	size_t id = 0;
    GraphicalPrimitives2D::Point2D * current, * previous = nullptr;

	for (size_t i=0; i<_all_points_array.GetCount(); i++) {
...
void PolygonSet::CyclesToPolygons(CycleSet *cycle_set)
{
	...
	if (cycle_set) {
		...
		for (size_t i=0; i<cycle_set->GetCount();i++){
			...
			if (cycle->GetVertexCount()>2) {
				...
				for (size_t j=0; j<cycle->GetVertexCount();j++){
...
*/
GraphicalPrimitives2D::Point2D * PolygonSet::PointByID(size_t id)
{
	GraphicalPrimitives2D::Point2D *p;

	// note: the position is at least equal to id
	for (size_t i=id; i<_all_points_array.GetCount();i++) {
...
*/
wxString PolygonDetection::PolygonSet::AsSVG(bool colorized)
{
	wxString result = "";
	...
	if (_polygons_array.GetCount()>0)
		delta=1.0f/(_polygons_array.GetCount()+1);

	float r, g, b;
	Color::HSVtoRGB(&r,&g,&b, POLYGON_COLOR_H, POLYGON_COLOR_S, POLYGON_COLOR_V);

	// draw polygons
	for (size_t i=0;i<_polygons_array.GetCount();i++) {
		Polygon * p = Item(i);
		if (colorized)
